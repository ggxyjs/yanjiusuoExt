				...
				else
				{
					...
					if(jsonData != null && jsonData != "")
					{
						...
						$(jsonData).each(function(idx,item){
							ImgView +='<div class="item enabled editEnabled" data-extname="docx" data-category="4" node-type="item"><a href="'+basePath +'pages/ReadOnline/ReadOnline.do?filename='+item.filename+'&dir=mis"><div title="'+item.filename+'" class="thumb-large global-icon-62 global-icon-62-'+item.filefix+'"><span class="item-chk" node-type="item-chk"></span></div></a><div class="file-name"><a href="'+basePath +'pages/ReadOnline/ReadOnline.do?filename='+item.filename+'&dir=mis" node-type="name">'+item.filename+'</a></div></div>';
						});
			...
			if((null==result)||(""==result)){
				...
				if(data!=[] && data!="" && data.length!=0){
					...
					for(var i=start;i<end;i++){
						var obj = data[i];
						FileHtml +='  <div class="item global-clearfix"><div style="width: 40%" class="col c1"><a href="'+basePath +'pages/DownLoad/DownloadFile.do?filename='+obj.filename+'&dir=mis">'+obj.filename+'</a></div><div style="width: 16%;" class="col">'+obj.size+'</div><div style="width: 23%;" class="col">'+obj.modify+'</div><div style="width: 20%;" class="col"><a href="'+basePath +'pages/DownLoad/DownloadFile.do?filename='+obj.filename+'&dir=mis">点击下载</a></div></div>';
					}
				}
